...
angular.module('groupModule').factory('groupSrvc', [ 'Restangular', '$q',
	...
	return{
		...
		getFullgroupList: function(startPage, pageLen){
			return Restangular.all(ORG+'/public_group/items').getList(
					{start:startPage, len:pageLen, extent:"full"});
		},
		getExtendgroupList: function(){
			return Restangular.all(ORG+'/public_group/items').getList({len:-1});
		},
		getgroupList: function(){
			return Restangular.all(ORG+'/public_group/items').getList();
		},
		getgroup: function(uid){
			return Restangular.one(ORG+'/public_group/items/'+uid).get();
		},
		savegroup: function(data){
			return Restangular.all(ORG+'/public_group').post(data);
		},
		updategroup: function(data){
			var uid = data.uid;
			return Restangular.all(ORG+'/public_group/items/'+uid).post(data);
		},
		deletegroup: function(uid){
			return Restangular.one(ORG+'/public_group/items/'+uid).remove();
		},
		searchgroup: function(query, startPage, pageLen){
			return Restangular.all(ORG+'/public_group/items').customPOST(query,'',
