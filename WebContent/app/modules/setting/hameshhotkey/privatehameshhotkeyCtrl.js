angular.module('privatehameshhotkeyModule').controller('privatehameshhotkeyCtrl', function($scope, privatehameshhotkeySrvc) {
	...
	$scope.Func = {
		...
		onSelecthameshhotkey: function(hameshhotkey){
			privatehameshhotkeySrvc.gethameshhotkey(hameshhotkey.uid).then(function(response){
		...
		onSavehameshhotkeyClick: function(){
			if($scope.hameshhotkeyForm.$valid){
				privatehameshhotkeySrvc.savehameshhotkey($scope.Data.selectedhameshhotkey).then(function(response){
		...
		onUpdatehameshhotkeyClick: function(){
			if($scope.hameshhotkeyForm.$valid){
				privatehameshhotkeySrvc.updatehameshhotkey($scope.Data.selectedhameshhotkey).then(function(response){
					$scope.Controller.hameshhotkeyListController.refreshList()
					$scope.Func.resetForm();
				});
			}else{
		...
		onDeletehameshhotkeyClick: function(){
			privatehameshhotkeySrvc.deletehameshhotkey($scope.Data.selectedhameshhotkey.uid).then(function(response){
				$scope.Func.resetForm();
				$scope.Func.reset();
				$scope.Controller.hameshhotkeyListController.refreshList();
			});
	...
	$scope.Controller = {
		hameshhotkeyListController : {
			...
			getList : privatehameshhotkeySrvc.getFullhameshhotkeyList,
			onListItemSelect : $scope.Func.onSelecthameshhotkey,
			searchFunction : privatehameshhotkeySrvc.searchhameshhotkey,
		},
		...
		hameshhotkeyAdvancedSearchController: {
			...
			searchableFieldInfo: [
				...
				{key:"order", type:"disabled", label:"ترتیب"},
			],
			onSearchClick: $scope.Func.onSearchClick,
			onExitSearchModeClick: $scope.Func.onExitSearchModeClick
		}
