angular.module('privatetagModule').controller('privatetagCtrl', function($scope, privatetagSrvc, viraTreeSrvc, $modal) {
	$scope.treeData = {
		selectedNode : {},
	}
	...
	$scope.Func = {
		...
		onUpdatetagClick : function() {
			if ($scope.tagForm.$valid) {
				...
				privatetagSrvc.updatetag(sendingData).then(function(response) {
		...
		onDeletetagClick : function() {
			privatetagSrvc.deletetag($scope.treeData.selectedNode.uid).then(function(response) {
			...
			var modalInstance = $modal.open({
				...
				resolve : {
					...
					getTopicTree: function () {
						return privatetagSrvc.gettagChildren
					},
					getFullTree: function () {
						return privatetagSrvc.getFullTree
					},
					getChildrenFn: function () {
						return privatetagSrvc.getChildren
					}
				}
			});

			modalInstance.result.then(function(newParentNode) {
				privatetagSrvc.changeParent({
		...
		getGeoRegionRootTree : function() {
			return privatetagSrvc.gettagChildren().then(function(response) {
				return {
					rootUid: "root-geo",
					data : response.data
				}
		...
		getGeoRegionChildrenTree : function(nodeUID) {
			return privatetagSrvc.getChildren(nodeUID);
		},
		remove : function(uid) {
			return privatetagSrvc.deletetag(uid);
		},
		select : function(uid) {
			return privatetagSrvc.gettag(uid).then(function (response) {
		...
		searchActions : {
			...
			onSearchClick : function() {
				...
				return privatetagSrvc.searchOnTree(searchItem).then(function (response) {
	...
	$scope.Controller = {
		...
		viraTree : {
			...
			addNodeFn : function(sendingObj, parentUid) {
				return privatetagSrvc.savetag(sendingObj, parentUid);
	...
	var Run = function() {
		// $scope.Controller.tagTreeController.modalTreeControllerObj = angular.copy($scope.Controller.tagTreeController);
	}
