...
///////////////////////////////////FIXME rahimi
			...
			$scope.Func = {
				...
				doSomeOnServerFormInfo: function (response) {
					...
					$timeout(function () {
						...
						if (_.isObject($scope.Data.letter.externalOrganizationSender)) {
							$scope.Func.onSelectOrganizationSender('', $scope.Data.letter.externalOrganizationSender);
				...
				getSenderList: function (externalOrgUid) {
					secretariatSrvc.getSenderSecretariatList($scope.Data.orgUid, externalOrgUid).then(function (response) {
						...
						if($scope.Data.senderList.length) {
							$scope.Data.letter.sender = $scope.Data.senderList[0];
						}
					});
				},
			...
			$scope.controller = {
				...
				letterBody: {
					onPdfUploaded : function (file) {
						// delete $scope.controller.vtPDF.pdfUrl;
					}
				},
