angular.module('NationalId.Directive', []).directive(
		"nationalcode",
		function() {
			return {
				require : 'ngModel',
				link : function(scope, elem, attr, ngModel) {
					...
					ngModel.$parsers.unshift(function(code) {
						...
						if (code.length < 8 || parseInt(code, 10) == 0){
							ngModel.$setValidity('nationalcode', false);
							return code;
						}
						...
						ngModel.$setValidity('nationalcode', valid);
						console.log(valid)
						return code;
					});

					// For model -> DOM validation
					ngModel.$formatters.unshift(function(code) {
						
						if (!!!code || code.length < 8 || parseInt(code, 10) == 0){
							ngModel.$setValidity('nationalcode',false);
							return code;
						}
