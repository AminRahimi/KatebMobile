...
.controller('ui.bootstrap.persian.datepicker.DatepickerController', [ '$scope', '$attrs', '$parse', '$interpolate', '$timeout', '$log', 'dateFilter', 'datepickerConfig', 'PersianDateService', 'persianDateFilter', function($scope, $attrs, $parse, $interpolate, $timeout, $log, dateFilter, datepickerConfig, PersianDateService, persianDateFilter) {
	...
	$scope.move = function(direction) {
		...
		var newDate = PersianDateService.addJMonthToGregDate(self.activeDate, direction * (self.step.months || 0));
		newDate = PersianDateService.addJYearToGregDate(newDate, direction * (self.step.years || 0));
...
.directive('persianDaypicker', [ 'dateFilter', 'PersianDateService', 'persianDateFilter', function(dateFilter, PersianDateService, persianDateFilter) {
	return {
		...
		link : function(scope, element, attrs, ctrl) {
			...
			// var DAYS_IN_MONTH = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30,
			// 31];
			// function getDaysInMonth( year, month ) {
			// return ((month === 1) && (year % 4 === 0) && ((year % 100 !== 0)
			// || (year % 400 === 0))) ? 29 : DAYS_IN_MONTH[month];
			// }

			function getDates(startDate, n) {
				var dates = new Array(n), current = new Date(startDate), i = 0;
				current.setHours(12); // Prevent repeated dates because of
										// timezone bug
				while (i < n) {
			...
			ctrl._refreshView = function() {
				...
				firstDayOfMonth = PersianDateService.persian_to_gregorian_Date(year, month + 1, 1),

				difference = ctrl.startingDay - firstDayOfMonth.getDay(), numDisplayedFromPreviousMonth = (difference > 0) ? 7 - difference : -difference, firstDate = new Date(firstDayOfMonth);
				...
				// scope.title = dateFilter(ctrl.activeDate,
				// ctrl.formatDayTitle);
			...
			ctrl.handleKeyDown = function(key, evt) {
				...
				if (key === 'left') {
					...
					// date =
					// Math.min(getDaysInMonth(ctrl.activeDate.getFullYear(),
					// ctrl.activeDate.getMonth()), date);
					date = Math.min(PersianDateService.persianMonthDays(PersianDateService.getFullYear(ctrl.activeDate), PersianDateService.getMonth(ctrl.activeDate) + 1), date);

				} else if (key === 'home') {
					date = 1;
				} else if (key === 'end') {
					// date = getDaysInMonth(ctrl.activeDate.getFullYear(),
					// ctrl.activeDate.getMonth());
					date = PersianDateService.persianMonthDays(PersianDateService.getFullYear(ctrl.activeDate), PersianDateService.getMonth(ctrl.activeDate) + 1);
				}
	...
	return {
		...
		link : function(scope, element, attrs, ctrl) {
			...
			ctrl._refreshView = function() {
				...
				for (var i = 0; i < 12; i++) {
					// months[i] = angular.extend(ctrl.createDateObject(new
					// Date(year, i, 1), ctrl.formatMonth), {
					months[i] = angular.extend(ctrl.createDateObject(PersianDateService.persian_to_gregorian_Date(year, i + 1, 2), ctrl.formatMonth), {
						uid : scope.uniqueId + '-' + i
					});
				}
		...
		link : function(scope, element, attrs, ctrl) {
			...
			ctrl._refreshView = function() {
				...
				for (var i = 0, start = getStartingYear(PersianDateService.getFullYear(ctrl.activeDate)); i < range; i++) {
					...
					// years[i] = angular.extend(ctrl.createDateObject(new
					// Date(start + i, 0, 1), ctrl.formatYear), {
					years[i] = angular.extend(ctrl.createDateObject(PersianDateService.persian_to_gregorian_Date(start + i, 1, 2), ctrl.formatYear), {
	...
	return {
		...
		link : function(scope, element, attrs, ngModel) {
			...
			if (attrs.dateDisabled) {
				datepickerEl.attr('date-disabled', 'dateDisabled({ date: date, mode: mode, activeDate: activeDate})');
			}
			function parseDate(viewValue) {
				if (!viewValue) {
					...
					// var date = dateParser.parse(viewValue, dateFormat) || new
					// Date(viewValue);
					var dateArray = viewValue.split("/")
					var date = PersianDateService.persian_to_gregorian_Date(parseInt(dateArray[0]), parseInt(dateArray[1]), parseInt(dateArray[2])) || new Date(viewValue);
					if (isNaN(date)) {
			...
			ngModel.$render = function() {
				...
				var date = ngModel.$viewValue ? persianDateFilter(ngModel.$modelValue, dateFormat) : '';
				element.val(date);
...
angular.module("template/persianDatepicker/datepicker.html", []).run(
		[
				"$templateCache",
				function($templateCache) {
					$templateCache.put("template/persianDatepicker/datepicker.html", "<div ng-switch=\"datepickerMode\" role=\"application\" ng-keydown=\"keydown($event)\">\n" + "  <persian-daypicker ng-switch-when=\"day\" tabindex=\"0\"></persian-daypicker>\n" + "  <persian-monthpicker ng-switch-when=\"month\" tabindex=\"0\"></persian-monthpicker>\n"
							+ "  <persian-yearpicker ng-switch-when=\"year\" tabindex=\"0\"></persian-yearpicker>\n" + "</div>");
				} ]);

angular.module("template/persianDatepicker/day.html", []).run(
		[
				"$templateCache",
				function($templateCache) {
					$templateCache.put("template/persianDatepicker/day.html", "<table role=\"grid\" aria-labelledby=\"{{uniqueId}}-title\" aria-activedescendant=\"{{activeDateId}}\">\n" + "  <thead>\n" + "    <tr>\n"
...
angular.module("template/persianDatepicker/month.html", []).run(
		[
				"$templateCache",
				function($templateCache) {
					$templateCache.put("template/persianDatepicker/month.html", "<table role=\"grid\" aria-labelledby=\"{{uniqueId}}-title\" aria-activedescendant=\"{{activeDateId}}\">\n" + "  <thead>\n" + "    <tr>\n"
							+ "      <th><button type=\"button\" class=\"btn btn-default btn-sm pull-right\" ng-click=\"move(-1)\" tabindex=\"-1\"><i class=\"glyphicon glyphicon-chevron-right\"></i></button></th>\n"
...
angular.module("template/persianDatepicker/popup.html", []).run(
		[
				"$templateCache",
				function($templateCache) {
					$templateCache.put("template/persianDatepicker/popup.html", "<ul style=\"direction:rtl;\"  class=\"dropdown-menu datepicker-popup\" ng-style=\"{display: (isOpen && 'block') || 'none', top: position.top+'px', left: position.left+'px'}\" ng-keydown=\"keydown($event)\">\n" + "	<li ng-transclude  style=\"padding: 5px 9px;\"></li>\n"
							+ "	<li ng-if=\"showButtonBar\" style=\"padding:10px 9px 2px\">\n" + "		<span class=\"btn-group\">\n" + "			<button type=\"button\" class=\"btn btn-sm btn-info\" ng-click=\"select('today')\">{{ getText('current') }}</button>\n" + "			<button type=\"button\" class=\"btn btn-sm btn-danger\" ng-click=\"select(null)\">{{ getText('clear') }}</button>\n" + "		</span>\n"
							+ "		<button type=\"button\" class=\"btn btn-sm btn-success pull-left\" ng-click=\"close()\">{{ getText('close') }}</button>\n" + "	</li>\n" + "</ul>\n" + "");
				} ]);

angular.module("template/persianDatepicker/year.html", []).run(
		[
				"$templateCache",
				function($templateCache) {
					$templateCache.put("template/persianDatepicker/year.html", "<table role=\"grid\" aria-labelledby=\"{{uniqueId}}-title\" aria-activedescendant=\"{{activeDateId}}\">\n" + "  <thead>\n" + "    <tr>\n"
							+ "      <th><button type=\"button\" class=\"btn btn-default btn-sm pull-right\" ng-click=\"move(-1)\" tabindex=\"-1\"><i class=\"glyphicon glyphicon-chevron-right\"></i></button></th>\n"
