angular.module("viraTree").factory('viraTreeSrvc', function() {
	var viraTreeSrvc = {};
	viraTreeSrvc = {
		...
		getChildsAndPushToTree : function(collapsingTreeData, getChildrenFn, beforeSelectedNodes, callBackFnIfNoChildNeedToGet) {
			...
			if (collapsingTreeData.childCount) {
				if (!collapsingTreeData.children.length) {
					getChildrenFn(collapsingTreeData.uid ? collapsingTreeData.uid : collapsingTreeData._uid).then(function(response) {
						if (!response.data.hasOwnProperty("children")) {
							response.data.children = response.data;
						}
						var treeData = response.data;
						var treeData = new viraTreeSrvc.tree(treeData, beforeSelectedNodes);
		...
		tree : function(data, beforeSelectedNodes, isFullOpen) {
			...
			if (data.children && data.children.length) {
				for (var i = 0; i < data.children.length; i++) {
					this.children.push(new viraTreeSrvc.tree(data.children[i], beforeSelectedNodes, isFullOpen));
				}
			}
