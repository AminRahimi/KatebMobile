angular.module("viraTree", []).directive('viraTree', function(viraTreeSrvc) {
	return {
		restrict : 'EA',
		// replace: true,
		templateUrl : 'app/assets/js/directives/viraTree/viraTreeTemplate.html',
		scope : {
			isEditable : "=",
			selectedNode : "=",
			selectedNodes : "=",
		...
		controller : function($scope, $rootScope, $modal, $q, $timeout) {
			$scope.treeItemRendererAddress = 'app/assets/js/directives/viraTree/tree_item_renderer.html';
			$scope.tempNode;
			...
			$scope.controller.loadTree = function(uid) {
				return $scope.controller.getTopicTree(uid).then(function(response) {
					...
					$scope.treeData = new viraTreeSrvc.tree($scope.treeData);
			...
			var getTopicTree = function(isFullTree, isWithStats, uid) {
				...
				if (isFullTree) {
					return $scope.controller.getFullTree(isWithStats, uid).then(function(response) {
						...
						$scope.treeData = new viraTreeSrvc.tree(treeData, [], true);
						if ($scope.controller.onInit) {
							$scope.controller.onInit();
						}
					});
			...
			$scope.onSelectNodeClick = function(node) {
			...
			$scope.onCollapseClick = function(collapsingTreeData) {
				...
				return viraTreeSrvc.getChildsAndPushToTree(collapsingTreeData, $scope.controller.getChildrenFn, beforeSelectedNodes, (function(_collapsingTreeData) {
					return function() {
			...
			$scope.controller.onSaveNewNodeClick = function(sendingObj, parentNode, callBackFn, secretLevel) {
				...
				var addingNode = viraTreeSrvc.generateNewNode(sendingObj);
				...
				$scope.controller.addNodeFn(sendingObj, parentNode.uid).then(function(response) {
					...
					var treeData = new viraTreeSrvc.tree(addingNode);
					treeData = [ treeData ];
					...
					viraTreeSrvc.getChildsAndPushToTree(parentNode, $scope.controller.getChildrenFn, beforeSelectedNodes, (function(_viraTreeSrvc, _parentNode, _treeData) {
						return function() {
							_viraTreeSrvc.pushNodeToChildOfNode(_parentNode, _treeData);
						};
					})(viraTreeSrvc, parentNode, treeData));
			...
			$scope.controller.onNodeArrayClick = function(searchingNodes, isJustFind) {
				if (!angular.isString($scope.treeData.uid) && $scope.treeData.children.length == 1) {
					viraTreeSrvc.findNodesExpandAndGetChildsAndSelect($scope.treeData.children[0], searchingNodes, selectNode, $scope.controller.getTopicTreeChildsFn);
				} else {
					viraTreeSrvc.findNodesExpandAndGetChildsAndSelect($scope.treeData, searchingNodes, selectNode, $scope.controller.getTopicTreeChildsFn, isJustFind, goToNodeLocation);
				}
			...
			$scope.controller.onDeleteNodeClick = function() {
				...
				//return topicSrvc.deleteNode(selectedNode.uid).then(function() {
					var node = viraTreeSrvc.removeNodeFromTree($scope.treeData, selectedNode);
			...
			$scope.onChangeParentClick = function(node) {
				...
				modalInstance.result.then(function(newParentNode) {
					topicSrvc.setNewParentForNode(selectedNode.uid, selectedNode.name, newParentNode.uid).then(function() {
						viraTreeSrvc.removeNodeFromTree($scope.treeData, selectedNode);
						selectedNode.indexInParent = newParentNode.childCount + 51841;
						viraTreeSrvc.findNodeAndPushChildToIt($scope.treeData, newParentNode.uid, [ selectedNode ], true);
					});
				}, function() {
				});
			};
			...
			$scope.onChangeIndexClick = function(node, factor) {
				...
				var parentNode = viraTreeSrvc.findParentOfNode($scope.treeData, node.uid);
				if (factor === 1) {
			...
			$scope.controller.getTreeAndSelectNode = function(nodeUid, nodes, isMulti) {
				...
				$scope.controller.getTopicTree(nodeUid).then(function(response) {
					...
					$scope.treeData = new viraTreeSrvc.tree(treeData, nodes);
